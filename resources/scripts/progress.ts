...
function addEventListeners(delay: number) {
...
function progress(event: any) {
    if (NProgress.isStarted() && event.detail.progress.percentage) {
        NProgress.set(Math.max(NProgress.status ?? 0, event.detail.progress.percentage / 100 * 0.9))
    }
}
...
export default InertiaProgress
